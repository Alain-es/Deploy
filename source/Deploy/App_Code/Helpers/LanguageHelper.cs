...
{
    // Code taken from: http://our.umbraco.org/forum/umbraco-7/developing-umbraco-7-packages/46770-Creating-new-Sections-Can-we-auto-generate-dictionary-item?p=0

    /// <summary>
    /// Checks and/or installs language settings on the first request
    /// </summary>
    public class LanguageHelper
    {
        ...
        /// Check each label and if not in the Umbraco language file, add it to the actions node
        /// </summary>
        public static void CheckAndInstallLanguageActions()
        {
            if (!_executed)
            {
                InstallLanguageKey("sections", "Deploy", "Deploy");
                _executed = true;
            }
        }
        ...
        private static void InstallLanguageKey(string area, string key, string value)
        {
            if (KeyMissing(area, key))
            {
                ...
                foreach (var languagefile in languageFiles)
                {
                    ...
                    {
                        LogHelper.Error<LanguageHelper>("Error in language installer", ex);
                    }
                }
            }
        }
